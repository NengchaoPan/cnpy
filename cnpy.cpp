...
void cnpy::parse_npy_header(FILE* fp, size_t& word_size, std::vector<size_t>& shape, bool& fortran_order, char& data_type) {
    char buffer[256];
    size_t res = fread(buffer,sizeof(char),11,fp);
    if(res != 11)
        throw std::runtime_error("parse_npy_header: failed fread");
...
cnpy::NpyArray load_the_npy_file(FILE* fp) {
    ...
    cnpy::parse_npy_header(fp,word_size,shape,fortran_order, data_type);

    //cnpy::NpyArray arr(shape, word_size, fortran_order);
    cnpy::NpyArray arr(shape, word_size, fortran_order,data_type);
